...
def define_variables(stockcodes):
    ...
    # stocks = [(f's_{stk}') for stk in stockcodes] ??
...
def define_cqm(stocks, num_stocks_to_buy, returns):
    ...
    # cqm.add_constraint_from_iterable([(stk, 1) for stk in stocks], "==", num_stocks_to_buy, label="choose k stocks") ??
    ...
    # cqm.objective.add_linear_from(*zip(stocks, returns), -1) ??
